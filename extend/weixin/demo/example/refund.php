...
if(isset($_REQUEST["transaction_id"]) && $_REQUEST["transaction_id"] != ""){
	$transaction_id = $_REQUEST["transaction_id"];    //微信 订单号
	$total_fee = $_REQUEST["total_fee"];                //订单总金额
	$refund_fee = $_REQUEST["refund_fee"];                  //申请 退款的金额
	$input = new WxPayRefund();
	$input->SetTransaction_id($transaction_id);
	$input->SetTotal_fee($total_fee);
	$input->SetRefund_fee($refund_fee);
    $input->SetOut_refund_no(WxPayConfig::MCHID.date("YmdHis"));   //退款单号  是自定义 的
    $input->SetOp_user_id(WxPayConfig::MCHID);                          //操作员 默认是商户
...
if(isset($_REQUEST["out_trade_no"]) && $_REQUEST["out_trade_no"] != ""){  //用商户 订单号申请退款
